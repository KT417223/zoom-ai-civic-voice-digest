...
これはアクセシビリティの取り組みとしてとても大事だと思っています。広報誌の内容を読むのが難しい人にも情報が届きますし、耳で聞ける形になっているのはありがたいです。
...
ただ、調べながら進めるうちに、この前提が少し不確かだと感じました。

Zoom AI Servicesが、任意の既存MP3ファイルや公開音声URLをそのまま受け取って文字起こしできるとは限らないためです。Zoomは会議や録画、そこで生成される文字起こし・要約とは相性が良さそうですが、「汎用の音声ファイル文字起こしAPI」として使えるかは別問題です。
...
ここが崩れると、記事も実装も丸ごと組み直しになります。入口のAPI仕様を確認しないまま進めるのは危ないと判断しました。

自分が調べた範囲では、Zoom AI Servicesに任意の既存音声ファイルを直接取り込ませる方法は見つけられていません。もし正式に取り込む方法がある場合は、ぜひ教えてください。
...
つまり、今回の主役は「音声ファイルをどう入力するか」ではなく、**文字起こし済みの広報情報を、住民向けに読みやすく整える部分**です。
...
このあたりは地味ですが、自治体情報を扱うなら大事なところだと思っています。
...
自治体情報は、それらしく要約できればOKというものではありません。施設名、制度名、申込期限、対象者を間違えると困ります。
...
ここでは本物の自治体情報のように見えすぎないようにしています。記事に載せる場合も、ドライランのサンプルであることを明示します。
...
自治体のお知らせで「対象者が違いました」「期限が違いました」となると影響が大きいので、ここはAI任せにしない設計にします。
...
自治体の音声広報を多言語ダイジェスト化するアイデアは、住民向け情報アクセス、多言語対応、アクセシビリティの面で十分に実用性がありそうです。
